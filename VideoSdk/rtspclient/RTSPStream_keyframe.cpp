...
int RTSPSession::startRTSPClient(char const* progName, char const* rtspURL, int debugLevel)
{
	...
	//sys_os_create_thread((void *)sip_timer_task, NULL)
		...
		if (m_nIndex >= 0)
		{
			...
			if (m_blFirstFrame)
			{
				if (strcmp(fSubsession.codecName(), "H264") == 0 || strcmp(fSubsession.codecName(), "h264") == 0)
				{
					...
					if (m_lsObj)
					{
						//RTP_BUFFER tbuf = { 11, m_nRecvNum, tmpbuf, 1.0f };
				...
				else if (strcmp(fSubsession.codecName(), "H265") == 0 || strcmp(fSubsession.codecName(), "h265") == 0)
				{
					...
					if (m_lsObj) 
					{
						//RTP_BUFFER tbuf = {11, npos, tmpbuf, 1.0f };
					...
					if (m_blFirstKeyFrame == false)
					{
						...
						if (keyframe == 1)
						{
							...
							if (m_lsObj)
							{
								...
								//envir() << "----keyframe -push_back 264 head " << tbuf.uLen << " [" << sRet.c_str() << "] m_lsObj=" << m_lsObj << "\r\n";
								//delete tmpMsg;
							}
							...
							{
								//rtp数据压栈
								//RTP_BUFFER tbuf = {1, m_nRecvNum, listbuf, 1.0f };
								RTP_BUFFER tbuf = { 1, m_nRecvNum, listbuf};
								m_lsObj->pushObj(tbuf);
								//envir() << "---264--push_back key data " << tbuf.uLen << " m_lsObj=" << m_lsObj << "\r\n";
						...
						{
							//envir() << "---264 not m_blFirstKeyFrame data " << frameSize << " \n";
							log_print(MHC_LOG_DEBUG, "%s %d, ---264 not m_blFirstKeyFrame data[%d] \n", __FUNCTION__, __LINE__, frameSize);
						}
					}
					else
					{
					...
					if (m_blFirstKeyFrame == false)
					{
						...
						//if (keyframe == 1)
						{
							...
							if (m_lsObj)
							{
								...
								log_print(MHC_LOG_DEBUG, "%s %d, -----push_back key data[%d] \n", __FUNCTION__, __LINE__, tbuf.uLen);
						...
						//else
						//{
						//	envir() << "--- 265 not m_blFirstKeyFrame data " << frameSize << " \n";
						//	log_print(MHC_LOG_DEBUG, "%s %d, --- 265 not m_blFirstKeyFrame data[%d] \n", __FUNCTION__, __LINE__, frameSize);
						//}
					}
					else
					{
...
#else
							RTP_BUFFER tbuf = { (uint32_t)keyframe, frameSize, fReceiveBuffer };
#endif
							m_lsObj->pushObj(tbuf);
