...
void * sys_os_create_mutex()
{
    ...
    {
        log_print(MHC_LOG_ERROR, "%s, sem_open failed. name=%s\r\n", __FUNCTION__, name);
        return NULL;
    }
...
pthread_t sys_os_create_thread(void * thread_func, void * argv)
{
	...
	if (ret != 0)
	{
		log_print(MHC_LOG_ERROR, "%s, pthread_create failed, ret = %d\r\n", __FUNCTION__, ret);
	}
	...
	if (hret == NULL || tid == 0)
	{
		log_print(MHC_LOG_ERROR, "%s, CreateThread hret=%u, tid=%u, err=%u\r\n", __FUNCTION__, hret, tid, GetLastError());
	}
...
int _mem_log_print(char *fmt, va_list argptr)
{
	...
	if (pMemLogFile == NULL)
	{
		...
		{
			log_print(MHC_LOG_ERROR, "%s, log init sem_init failed[%s]\r\n", __FUNCTION__, strerror(errno));
		}
	}
