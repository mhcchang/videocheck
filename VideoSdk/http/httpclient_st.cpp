...
#include "httpclient_st.h"

// 初始化client静态变量
int HttpClient_St::s_exit_flag = 0;
ReqCallback HttpClient_St::s_req_callback;
std::string HttpClient_St::s_post_data = "";
std::string HttpClient_St::s_url = "";
std::string HttpClient_St::s_resp = "";

// 客户端的网络请求响应
void HttpClient_St::OnHttpEvent(mg_connection *connection, int ev, void *http_req, void *fn_data)//(mg_connection *connection, int ev, int event_type, void *event_data)
{
...
void HttpClient_St::SendReq(const std::string url, const std::string extra_headers,
	const std::string post_data, ReqCallback req_callback)
{
	...
	mg_connection* connection = mg_http_connect(&mgr, url.c_str(), HttpClient::OnHttpEvent, (void*)&mgr);
...
void HttpClient_St::SyncSendReq(const std::string url, const std::string extra_headers,
